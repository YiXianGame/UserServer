...
namespace Material.EtherealC.Service
{
    public class RPCServiceFactory
    {
        ...
        public static void Register<T>(string servicename, string hostname, string port, RPCServiceConfig config) where T : new()
        {
            Register(new T(), servicename, hostname, port, config);
        }
        public static void Register(object instance,string servicename,string hostname, string port, RPCServiceConfig config)
        {
